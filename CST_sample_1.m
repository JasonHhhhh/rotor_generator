function airfoil_1_data=CST_sample_1(doe_w,num,after_0_sample)
%input:doe_w: design of experiment matrix 
%      num: 0 or 1 which represents the tip or root airfoil  
%      after_0_sample: the output structure after tip airfoil sampling
dz=0.003;
N=150;
%initialization
CST_doe_Y=[];
        ...
        if loc_tmax<0.425
            if tmax<-0.08
                if t(0.65)<-0.01
                    ...
                    str=[str2,str1];
                    e=size(CST_doe_N,1);
                    CST_doe_N(e+1,:)=after_0_sample(i);
                end
            else
                str2='profile_N';
                str=[str2,str1];
...
airfoil_1_data.doe=doe;
airfoil_1_data.all_points=pp;
airfoil_1_data.Y=CST_doe_Y;
airfoil_1_data.N=CST_doe_N;
airfoil_1_data.scatter=foil;
eval(['save airfoil_data_',num2str(num),' airfoil_1_data'] )